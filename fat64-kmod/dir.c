...
/* Maximum buffer size of file name (with nul). */
#define FAT64_MAX_FN_SIZE 255 + 1
...
next:
	...
	*bh = NULL;
	...
	err = fat64_bmap(dir, iblock, &phys, &mapped_blocks, 0);
	if (err || !phys)
	...
	*bh = sb_bread(sb, phys);
	if (*bh == NULL) {
		fat64_msg_ratelimit(sb, KERN_ERR,
 ...
 */
int fat64_search_long(struct inode *inode, const unsigned char *name,
...
EXPORT_SYMBOL_GPL(fat64_search_long);
...
start_filldir:
	...
	if (!memcmp(de->name, MSDOS_DOT, MSDOS_NAME)) {
		...
		struct inode *tmp = fat64_iget(sb, i_pos);
		if (tmp) {
			inum = tmp->i_ino;
			iput(tmp);
		} else
			  ...
			  unsigned long arg)
{
	...
	case VFAT_IOCTL_READDIR_BOTH:
		...
		break;
	default:
		return fat64_generic_ioctl(filp, cmd, arg);
	}
				 ...
				 unsigned long arg)
{
	...
	case VFAT_IOCTL_READDIR_BOTH32:
		...
		break;
	default:
		return fat64_generic_ioctl(filp, cmd, (unsigned long)arg);
	}
...
const struct file_operations fat64_dir_operations = {
 ...
 */
int fat64_get_dotdot_entry(struct inode *dir, struct buffer_head **bh,
			 struct msdos_dir_entry **de)
{
	loff_t offset = 0;
...
EXPORT_SYMBOL_GPL(fat64_get_dotdot_entry);

/* See if directory is empty */
int fat64_dir_empty(struct inode *dir)
{
	struct buffer_head *bh;
	struct msdos_dir_entry *de;
	loff_t cpos;
...
EXPORT_SYMBOL_GPL(fat64_dir_empty);

/*
 * fat64_subdirs counts the number of sub-directories of dir. It can be run
 * on directories being created.
 */
int fat64_subdirs(struct inode *dir)
{
	struct buffer_head *bh;
	struct msdos_dir_entry *de;
	loff_t cpos;
 ...
 */
int fat64_scan(struct inode *dir, const unsigned char *name,
...
EXPORT_SYMBOL_GPL(fat64_scan);

/*
 * Scans a directory for a given logstart.
 * Returns an error code or zero.
 */
int fat64_scan_logstart(struct inode *dir, int i_logstart,
		      struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
...
static int __fat64_remove_entries(struct inode *dir, loff_t pos, int nr_slots)
{
	struct super_block *sb = dir->i_sb;
	struct buffer_head *bh;
...
int fat64_remove_entries(struct inode *dir, struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_dir_entry *de;
	struct buffer_head *bh;
	...
	if (nr_slots) {
		 ...
		 * the success)
		 */
		err = __fat64_remove_entries(dir, sinfo->slot_off, nr_slots);
		if (err) {
			fat64_msg(sb, KERN_WARNING,
	...
	if (IS_DIRSYNC(dir))
		(void)fat64_sync_inode(dir);
	else
		mark_inode_dirty(dir);

	return 0;
}
EXPORT_SYMBOL_GPL(fat64_remove_entries);
...
int fat64_alloc_new_dir(struct inode *dir, struct timespec *ts)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	...
	err = fat64_alloc_clusters(dir, &cluster, 1);
	if (err)
		goto error;
	...
	fat64_time_unix2fat(sbi, ts, &time, &date, &time_cs);
...
error_free:
	fat64_free_clusters(dir, cluster);
error:
	return err;
}
EXPORT_SYMBOL_GPL(fat64_alloc_new_dir);
			       ...
			       struct buffer_head **bh, loff_t *i_pos)
{
	...
	*nr_cluster = (size + (sbi->cluster_size - 1)) >> sbi->cluster_bits;
	...
	err = fat64_alloc_clusters(dir, cluster, *nr_cluster);
	if (err)
		goto error;
...
error_nomem:
	for (i = 0; i < n; i++)
		bforget(bhs[i]);
	fat64_free_clusters(dir, cluster[0]);
error:
	return err;
}

int fat64_add_entries(struct inode *dir, void *slots, int nr_slots,
		    struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	...
	if (dir->i_ino == MSDOS_ROOT_INO) {
		...
		fat64_msg(sb, KERN_ERR, "Corrupted directory (i_pos %lld)",
		       MSDOS_I(dir)->i_pos);
		err = -EIO;
		goto error;
	}
...
found:
	...
	if (free_slots) {
		...
		if (long_bhs && IS_DIRSYNC(dir))
			err = fat64_sync_bhs(bhs, long_bhs);
		if (!err && i < nr_bhs) {
	...
	if (nr_slots) {
		...
		err = fat64_chain_add(dir, cluster, nr_cluster);
		if (err) {
			fat64_free_clusters(dir, cluster);
			goto error_remove;
		}
		if (dir->i_size & (sbi->cluster_size - 1)) {
			fat64_fs_error(sb, "Odd directory size");
...
error_remove:
	brelse(bh);
	if (free_slots)
		__fat64_remove_entries(dir, pos, free_slots);
	return err;
}
EXPORT_SYMBOL_GPL(fat64_add_entries);
