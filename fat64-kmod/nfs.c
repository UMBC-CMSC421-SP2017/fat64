...
static struct inode *fat_dget(struct super_block *sb, int i_logstart)
{
	...
	head = sbi->dir_hashtable + fat64_dir_hash(i_logstart);
...
static struct inode *fat_ilookup(struct super_block *sb, u64 ino, loff_t i_pos)
{
	if (MSDOS_SB(sb)->options.nfs == FAT_NFS_NOSTALE_RO)
		return fat64_iget(sb, i_pos);

	else {
		if (ino < FAT64_ROOT_INO)
			return NULL;
				       ...
				       u64 ino, u32 generation, loff_t i_pos)
{
	...
	if (inode == NULL && MSDOS_SB(sb)->options.nfs == FAT_NFS_NOSTALE_RO) {
		...
		de = (struct fat64_dir_entry *)bh->b_data;
		...
		if (IS_FREE(de[offset].name))
			...
			inode = fat64_build_inode(sb, &de[offset], i_pos);
		brelse(bh);
	}
...
struct inode *fat_rebuild_parent(struct super_block *sb, int parent_logstart)
{
	...
	de = (struct fat64_dir_entry *) parent_bh->b_data;
	...
	if (!dummy_grand_parent) {
		...
		dummy_grand_parent->i_ino = iunique(sb, FAT64_ROOT_INO);
		fat64_fill_inode(dummy_grand_parent, &de[1]);
		MSDOS_I(dummy_grand_parent)->i_pos = -1;
	}

	if (!fat64_scan_logstart(dummy_grand_parent, clus_to_match, &sinfo))
		parent = fat64_build_inode(sb, sinfo.de, sinfo.i_pos);
...
static struct dentry *fat_get_parent(struct dentry *child_dir)
{
	...
	if (!fat64_get_dotdot_entry(d_inode(child_dir), &bh, &de)) {
...
const struct export_operations fat64_export_ops = {
	.fh_to_dentry   = fat_fh_to_dentry,
	.fh_to_parent   = fat_fh_to_parent,
	.get_parent     = fat_get_parent,
};

const struct export_operations fat64_export_ops_nostale = {
